...
void add_to_output_container(output_container& out, const output_type& t, fl min_rmsd, sz max_size) {
	...
	if(closest_rmsd.first < out.size() && closest_rmsd.second < min_rmsd) { 
		if(t.e < out[closest_rmsd.first].e) { 
			out[closest_rmsd.first] = t; // FIXME? slow <--Why keep duplicating objects? Use ref or ptr and update the orginal copy directly.
		}
	}
