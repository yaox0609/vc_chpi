...
struct parsed_atom : public atom_vc {
	...
	std::string atomname;
	std::string chainID;
	parsed_atom(sz ad_ , fl charge_, const std::string& resname, const std::string& resnum, const std::string& atomname, const vec& coords_, unsigned number_, const std::string& chainID_) :  number(number_) , resname(resname) , resnum(resnum) , atomname(atomname), chainID(chainID_) {
		ad = ad_;
		charge = charge_;
		coords = coords_;
	}
