...
void model::assign_types() {
	VINA_FOR(i, grid_atoms.size() + atoms.size()) {
		...
		switch(a.el) {
			...
			//case EL_TYPE_C    : x = bonded_to_heteroatom(a) ? XS_TYPE_C_P : (a.ad == AD_TYPE_A ? XS_TYPE_C_A : XS_TYPE_C_H); break; //This is Yao's modified code
...
fl  model::eval_deriv  (const precalculate& p, const igrid& ig, const vec& v, const conf& c, change& g/*, std::vector< std::vector<int> > glyco_info*/,const fl chi_coeff, const fl chi_cutoff ) { // clean up
	...
	//This CHpi term isn't really subject to "deriv" evaluation. What's the consequence?
	//e += (this->weight_chpi * eval_chpi());
...
void model::build_ar_ring_info() {
	...
	DetectAromaticCycles(this->receptor_residues, this->rec_ar_ring_info);

	VINA_FOR_IN(i, this->rec_ar_ring_info){
		ring_attribute& attr = this->rec_ar_ring_info[i];
		VINA_FOR_IN(j, attr.atom_indices){
			atom_vc& atom = this->grid_atoms[attr.atom_indices[j]];
			attr.atom_coord_ptrs.push_back(&(atom.coords));

			if (atom.ad == AD_TYPE_A){
				attr.aromatic_atom_ptrs.push_back(&atom);
				attr.aromatic_coord_ptrs.push_back(&(atom.coords));
			}
		}
		attr.ComputeRemainingAttributes();
	}

	this->lig_ar_ring_info.clear();
	DetectAromaticCycles(this->ligand_residues, this->lig_ar_ring_info);

	VINA_FOR_IN(i, this->lig_ar_ring_info){
		ring_attribute& attr = this->lig_ar_ring_info[i];
		
		VINA_FOR_IN(j, attr.atom_indices){
                        sz& ring_atom_index = attr.atom_indices[j];
                        attr.atom_coord_ptrs.push_back(&(this->coords[ring_atom_index]));
			atom_vc& atom = this->atoms[ring_atom_index];

			if (atom.ad == AD_TYPE_A){
                                attr.aromatic_atom_ptrs.push_back(&atom);
                                attr.aromatic_coord_ptrs.push_back(&(this->coords[ring_atom_index]));
                        }
                }
		attr.ComputeCentroid(); //Compute centroid only to compute effective radius. Centroid will be updated each time sf is called. 
		attr.ComputeEffectiveRadius(); //No need to compute normal here. Normal will be updated each time sf is called.
	}

	this->rec_ali_carb_info.clear();
	VINA_FOR_IN(i, this->grid_atoms){
		atom_vc& carbon = this->grid_atoms[i];
		if (carbon.ad != AD_TYPE_C) continue;

		vec* c_coord = &(carbon.coords);
		aptrv h_neighbors;
		vptrv h_coords;
		VINA_FOR_IN(j, carbon.bonds){
			const bond_vc& b = carbon.bonds[j];	
                	atom_vc& h = get_atom(b.connected_atom_index);
			if (h.ad != AD_TYPE_H) continue;
			h_neighbors.push_back(&h);
			h_coords.push_back(&(h.coords));
		}
		this->rec_ali_carb_info.push_back(aliphatic_carbon_attribute(&carbon, c_coord, h_neighbors, h_coords, this->chpi_explicit_hydrogen));
	}

	this->lig_ali_carb_info.clear();
        VINA_FOR_IN(i, this->atoms){
                atom_vc& carbon = this->atoms[i];
                if (carbon.ad != AD_TYPE_C) continue;
                ...
                VINA_FOR_IN(j, carbon.bonds){
                        ...
                        h_neighbors.push_back(&h);
			sz index = std::distance(this->ligand_atoms.begin(), std::find(this->ligand_atoms.begin(), this->ligand_atoms.end(), &h));
			VINA_CHECK(index < this->ligand_atoms.size());
                        h_coords.push_back(&(this->coords[index]));
                }
                this->lig_ali_carb_info.push_back(aliphatic_carbon_attribute(&carbon, c_coord, h_neighbors, h_coords, this->chpi_explicit_hydrogen));
        }
}
...
void model::DFSVisit(aptrv& atoms, std::map<atom_vc*, bool>& atom_visited_map, std::map<atom_vc*, atom_vc*>& atom_parent_map, atom_vc* atom, int& counter, aptrv& current_path, std::vector<aptrv>& detected_cycles){
	...
	VINA_FOR_IN(i, atom->bonds){
		...
		//If ligand is covalently bonded to receptor, use this to block searching to the other molecule. 
    		if (std::find(atoms.begin(), atoms.end(), n_ptr) != atoms.end()){
			//If not visited
				...
				if (num_neighbors_downstream > 1){
					std::map<atom_vc*, bool> child_atom_visited_map = atom_visited_map;
					std::map<atom_vc*, atom_vc*> child_atom_parent_map = atom_parent_map;
					aptrv child_current_path = current_path;
              				DFSVisit(atoms, child_atom_visited_map, child_atom_parent_map, n_ptr, counter, child_current_path, detected_cycles);
				}
				//If there is no branch, keep using containers parsed from upstream.
				else{
              				DFSVisit(atoms, atom_visited_map, atom_parent_map, n_ptr, counter, current_path, detected_cycles);
				}
                	}
                	else{
...
std::vector<aptrv> model::DetectCyclesByDFS(aptrv& atoms){
	int counter = 0;
	...
	std::vector<aptrv> cycles;
		...
		VINA_FOR_IN (i, atoms){
			atom_vc* aptr = atoms[i];
       			...
       			atom_parent_map[aptr] = &(fake_atom);
		}       

		atom_vc* first_atom = atoms[0];
       		DFSVisit(atoms, atom_visited_map, atom_parent_map, first_atom, counter, current_path, cycles);
       	}
...
void model::DetectAromaticCycles(resv& residues, ring_info& ring_attributes){
	VINA_FOR_IN(i, residues){
		aptrv& res_atoms = residues[i].atoms;
		std::vector<aptrv> ar_cycles_this_residue = DetectAromaticCycles(res_atoms);
		...
		VINA_FOR_IN(j, ar_cycles_this_residue){
			...
			VINA_FOR_IN(k, this_cycle){
				atom_vc*& atom = this_cycle[k];
				sz& index = residues[i].aptr_index_map[atom];
				indices.push_back(index);
			}
			ring_attributes.push_back(ring_attribute(indices, this_cycle));
		}
	}
	return;
}

std::vector<aptrv> model::DetectAromaticCycles(aptrv& atoms){

	std::vector<aptrv> cycles = DetectCyclesByDFS(atoms);
	...
	//std::cout << "After sorting for duplicate, num cycles: " << reasonable_cycles_non_duplicate.size() << std::endl;
        std::vector<aptrv> reasonable_cycles_non_redundant =  remove_redundant_cycles(reasonable_cycles_non_duplicate);
	//std::cout << "After sorting for redundant, num cycles: " << reasonable_cycles_non_redundant.size() << std::endl;

	std::vector<aptrv> aromatic_cycles_might_be_redundant;
	VINA_FOR_IN(i, reasonable_cycles_non_redundant){
		aptrv& ring_atoms = reasonable_cycles_non_redundant[i];
		aptrv ring_aromatic_atoms;

		VINA_FOR_IN(j, ring_atoms){
			atom_vc& atom = (*ring_atoms[j]);
			//If is aromatic carbon
			if (atom.xs == XS_TYPE_C_A_P || atom.xs == XS_TYPE_C_A_H){
				ring_aromatic_atoms.push_back(&atom);
			}
			//Else if not aliphatic carbon, check if it is a heteroatom (bonded to exactly two aromatic carbons)
			else if (atom.ad != AD_TYPE_C){
				int num_aromatic_carbon_neighbors = 0;
				VINA_FOR_IN(i, atom.bonds){
					const bond_vc& b = atom.bonds[i];
					atom_vc& neighbor = get_atom(b.connected_atom_index);
					if (neighbor.ad == AD_TYPE_A){
						num_aromatic_carbon_neighbors++;
					}
				}
				if (num_aromatic_carbon_neighbors == 2){
					ring_aromatic_atoms.push_back(&atom);
				}
			}
		}
		
		//Say, at least, 5-membered imidazole ring with 3 aromatic carbons (Histidine).
		if (ring_aromatic_atoms.size() >= 3){
			aromatic_cycles_might_be_redundant.push_back(ring_aromatic_atoms);
		}
       	}

	std::vector<aptrv> aromatic_cycles_non_redundant = remove_redundant_cycles(aromatic_cycles_might_be_redundant);

    	return aromatic_cycles_non_redundant;
}

fl model::eval_chpi_enthalpy_c(fl& r){
	//return chpi_const * std::pow(2.718, (-1) * (r-chpi_miu) * (r-chpi_miu) / (2* chpi_sigma * chpi_sigma)) / std::pow(2* 3.1415* chpi_sigma * chpi_sigma, 0.5);
	//fl g_numerator = -sqr(r-chpi_miu); 
	return g_coeff * std::exp(-sqr(r-chpi_miu)/g_denominator);

        //fl enthalpy = chpi_const * std::pow(2.718, (-1) * (r-chpi_miu) * (r-chpi_miu) / (2* chpi_sigma * chpi_sigma)) / std::pow(2* 3.1415* chpi_sigma * chpi_sigma, 0.5);
        //return enthalpy;
}
...
fl model::eval_chpi_entropy(fl& horizontal_offset){
	...
	//-TdS = -0.616*ln(horizontal_offset) + 0.2144 (0 ≤ r ≤ 1.4, R^2 = 0.998). Unit is kcal/mol, using S=-kb*p*ln(p) based on experimental data.
	return (-0.616 * std::log(std::max(horizontal_offset, chpi_ho_epsilon)) + 0.2144);  
}

fl model::eval_chpi_c_ring(aliphatic_carbon_attribute& c, ring_attribute& r){
	vec* c_coord = c.c_coord;
	fl(&cd)[3] = c_coord->data; //cd = carbon coord data
	bool this_ring_interacting = false;

	fl dH = 0.00;
	VINA_FOR_IN(i, r.aromatic_coord_ptrs){
		fl(&rd)[3] = r.aromatic_coord_ptrs[i]->data; //rd = ring atom coord data
		fl r = std::sqrt(sqr(cd[0]-rd[0]) + sqr(cd[1]-rd[1]) + sqr(cd[2]-rd[2]));
		if (r >= chpi_dcut) continue;

                this_ring_interacting = true;
                dH += eval_chpi_enthalpy_c(r);
	}

	return 0.00;

	if (!this_ring_interacting) return 0.00;

       	//Now, compute horizontal offset
	fl(&rcd)[3] = r.centroid.data;
	fl c_centroid[3];
	//This is the xyz of c-centroid vector
	c_centroid[0] = cd[0]-rcd[0]; c_centroid[1] = cd[1]-rcd[1]; c_centroid[2] = cd[2]-rcd[2];
	//This is the length of the c-centroid vector
	fl ccl = std::sqrt(c_centroid[0]*c_centroid[0] + c_centroid[1]*c_centroid[1] + c_centroid[2]*c_centroid[2]);
        if (ccl >= chpi_c_centroid_dcut) return dH;

	fl(&rnd)[3] = r.normal.data;
	//Vertical offset = abs(c_centroid * cos(theta)), since normal length = 1
        fl vo = std::abs(c_centroid[0]*rnd[0] + c_centroid[1]*rnd[1] + c_centroid[2]*rnd[2]);
        fl ho = std::sqrt(ccl*ccl - vo*vo);
        if (ho > chpi_ho_max) return dH;

	return dH; //Testing without entropy
	//return (dH - eval_chpi_entropy(ho));
}

fl model::eval_chpi_h_ring(aliphatic_carbon_attribute& c_attr, ring_attribute& r_attr){
	fl e = 0.00;
        vec* cv = c_attr.c_coord;
	vec& centroid = r_attr.centroid;
	vec& normal = r_attr.normal;
	...
	VINA_FOR_IN(i, c_attr.h_coords){
		vec* hv = c_attr.h_coords[i];
		//std::cout << "H coord xyz: " << hv->data[0] << "," << hv->data[1] << "," << hv->data[2] << std::endl;
		vec hc = centroid - (*hv);
		fl hcdp = hc * normal;
		...
		if (ccdp * hcdp < 0) continue;
                fl hcl = magnitude(hc); //H-centroid vector length
		...
		vec ch_bond; c_attr.ComputeCHBondVectors(i, ch_bond);
                fl chndp = ch_bond * normal; //CH-Normal dot product
                fl chl = magnitude(ch_bond); //CH length

        	fl dH = 0.00;
        	bool this_ring_interacting = false;

        	VINA_FOR_IN(j, r_attr.aromatic_coord_ptrs){
                	vec* rc = r_attr.aromatic_coord_ptrs[j];
                	fl r = std::sqrt(sqr(hv->data[0]-rc->data[0]) + sqr(hv->data[1]-rc->data[1]) + sqr(hv->data[2]-rc->data[2]));
                	if (r >= chpi_hcut) continue;

                	this_ring_interacting = true;
                	dH += eval_chpi_enthalpy_h(r);
        	}

        	fl TdS = 0.00;
		if (this_ring_interacting){
                	fl chnca = std::abs(chndp) / chl; //CH-Normal cosine absolute
			dH *= chnca; //Hydrogen interaction enthalpy is scaled by the cosine of CH-normal angle. 

			//First do a H-centroid distance cutoff, if greater, skip to save computation.
			if (hcl < chpi_h_centroid_dcut){
               			//Compute H-centroid horizontal offset for entropy.
                		fl hcho = std::sqrt(hcl*hcl - hcvo*hcvo);
                		VINA_CHECK(!std::isnan(hcho));

      	        		if (hcho < chpi_ho_max){
                			TdS = eval_chpi_entropy(hcho);
				}
			}
		}
...
fl model::eval_chpi(){
	VINA_FOR_IN(i, this->lig_ar_ring_info){
		this->lig_ar_ring_info[i].ComputeCentroidAndNormal();
	}
	if (this->chpi_explicit_hydrogen){
		return this->eval_chpi_h();
	}
	return this->eval_chpi_c();

}

fl model::eval_chpi_c(){
	fl lig_rec_chpi = 0.00;
	VINA_FOR_IN(i, this->lig_ar_ring_info){
		ring_attribute& r_inf = this->lig_ar_ring_info[i];
		VINA_FOR_IN(j, this->rec_ali_carb_info){
			lig_rec_chpi += eval_chpi_c_ring(this->rec_ali_carb_info[j], r_inf);
		}
	...
	VINA_FOR_IN(i, this->rec_ar_ring_info){
		...
		VINA_FOR_IN(j, this->lig_ali_carb_info){
                        rec_lig_chpi += eval_chpi_c_ring(this->lig_ali_carb_info[j], r_inf);
		}
	}
...
fl model::eval_chpi_h(){
        fl lig_rec_chpi = 0.00;
        VINA_FOR_IN(i, this->lig_ar_ring_info){
                ring_attribute& r_inf = this->lig_ar_ring_info[i];
                VINA_FOR_IN(j, this->rec_ali_carb_info){
                        lig_rec_chpi += eval_chpi_h_ring(this->rec_ali_carb_info[j], r_inf);
                }
                //Later put catpi, pipi, etc here. 
        }
        ...
        VINA_FOR_IN(i, this->rec_ar_ring_info){
                ring_attribute& r_inf = this->rec_ar_ring_info[i];
                VINA_FOR_IN(j, this->lig_ali_carb_info){
                        rec_lig_chpi += eval_chpi_h_ring(this->lig_ali_carb_info[j], r_inf);
                }
        }
...
fl model::eval_chpi_enthalpy_h(fl& r){
        //fl g_numerator_h = -sqr(r-chpi_miu);
        return g_coeff_h * std::exp(-sqr(r-chpi_miu_h)/g_denominator_h);
	
	//fl enthalpy = chpi_const_h * std::pow(2.718, (-1) * (r-chpi_miu_h) * (r-chpi_miu_h) / (2* chpi_sigma_h * chpi_sigma_h)) / std::pow(2* 3.1415* chpi_sigma_h * chpi_sigma_h, 0.5);
	//return enthalpy;
}

void model::build_residues_from_atoms(resv& residues, aptrv& atoms){
	VINA_FOR_IN(i, atoms){
                ...
                if (resnum != current_residue.number || chainID != current_residue.chainID){
                        residues.push_back(model_residue(resname, resnum, chainID));
                }

                model_residue& current_residue2 = residues.back();
                current_residue2.atoms.push_back(atom);
		current_residue2.aptr_index_map[atom] = i;
        }
}
void model::build_residue_info(){
	...
	this->build_residues_from_atoms(this->receptor_residues, this->grid_atom_ptrs);

	this->ligand_atoms.clear();
	for (unsigned int i = 0 ; i < this->atoms.size(); i++){
		atom_vc& atom = this->atoms[i];

		if (i < this->m_num_movable_atoms){
			this->ligand_atoms.push_back(&atom);
		}
	}

	this->ligand_residues.clear();
	this->build_residues_from_atoms(this->ligand_residues, this->ligand_atoms);
