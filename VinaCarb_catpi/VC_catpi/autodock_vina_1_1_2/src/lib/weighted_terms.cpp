	...
	VINA_FOR_IN(i, t->usable_terms)
		if(t->usable_terms.enabled[i]) {
			if(enabled_usable_terms.empty())
				{atom_typing_used_ = t->usable_terms[i].atom_typing_used;/*std::cout<<"\t weighted_terms.cpp weighted_terms:1\n";*/}
			else
				{VINA_CHECK(atom_typing_used_ == t->usable_terms[i].atom_typing_used);/*std::cout<<"\t weighted_terms.cpp weighted_terms:2\n";*/}
