...
struct catpi : public usable {
        fl good;
	fl bad;
        catpi(fl good_, fl bad_, fl cutoff_) : usable(cutoff_), good(good_), bad(bad_) {
                name = std::string("catpi(g=") + to_string(good) + ", b=" + to_string(bad) + ")";
        }
        fl eval(sz t1, sz t2, fl r) const {
		if(xs_catpi_possible(t1, t2)){
			double catpi_wt = 0.45, sigma = 0.5, miu = 3.8; 

			return catpi_wt * std::pow(2.718, (-1) * (r-miu) * (r-miu) / (2* sigma * sigma)) / std::pow(2* 3.1415* sigma * sigma, 0.5);
			//return slope_step(bad, good, r - optimal_distance(t1, t2));
		}
                return 0;   
        }
...
struct chpi : public usable {  //Yao's CH-pi function 1
	fl good;
	fl bad;
	chpi(fl good_, fl bad_, fl cutoff_) : usable(cutoff_), good(good_), bad(bad_) {
                name = std::string("chpi(g=") + to_string(good) + ", b=" + to_string(bad) + ")";
        }
        fl eval(sz t1, sz t2, fl r) const {
                fl chpi_cutoff = 4.5;
                if (r > chpi_cutoff)
                    return 0;
		else if (xs_chpi_possible(t1, t2))
                    return 1;
                return 0;
	}
};

struct chpi_stepsize : public usable {  //Yao's CH-pi function 2
        fl good;
        fl bad;
        chpi_stepsize(fl good_, fl bad_, fl cutoff_) : usable(cutoff_), good(good_), bad(bad_) {
                name = std::string("chpi_stepsize(g=") + to_string(good) + ", b=" + to_string(bad) + ")";
        }
        fl eval(sz t1, sz t2, fl r) const {
                if (xs_chpi_possible(t1, t2)){
                    return slope_step(bad, good, r); //Use the raw distance r, not the (r - optimal disance)
                }
                return 0;
        }
};

//double chpi_miu = 3.82, chpi_sigma = 0.40, chpi_const = 0.14; //Yao:Parameters in 2021
//double chpi_miu = 4.39, chpi_sigma = 0.62, chpi_const = 0.28; //Yao:Parameters in 2023, fit to complex A only
//double chpi_miu = 4.49, chpi_sigma = 0.75, chpi_const = 0.26; //Yao:Parameters in 2023, fit to complex ADE
struct chpi_gaussian : public usable { //Yao's CH-pi function 3
	fl good;
	fl bad;
        chpi_gaussian(fl good_, fl bad_, fl cutoff_) : usable(cutoff_), good(good_), bad(bad_) {
		name = std::string("chpi_gaussian(g=") + to_string(good) + ", b=" + to_string(bad) + ")";
	}
	fl eval(sz t1, sz t2, fl r) const {
...
/*struct chpi_ring_dependent : public usable {
	fl good;
	fl bad;
	chpi_ring_dependent(fl good_, fl bad_, fl cutoff_) : usable(cutoff_), good(good_), bad(bad_){
		name = "ring_dependent_chpi";
	}

	//This eval is not used, but exists for fuck's sake.
	fl eval(sz t1, sz t2, fl r) const {
		std::cout << "This function shouldn't ever get called." << std::endl;
		return 0;
	}
	fl evalchpi() const {
...
everything::everything() { // enabled according to design.out227
	...
	// additive
        ...
        add(1, new catpi(-0.2, -0.1, cutoff)); 
