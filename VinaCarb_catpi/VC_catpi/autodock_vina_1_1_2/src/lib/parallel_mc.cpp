...
struct parallel_mc_task {
	...
	parallel_mc_task(const model& m_, int seed) : m(m_), generator(static_cast<rng::result_type>(seed)) {
		//Yao 20230704: Keep using implicit constructor to copy objects. In my opinion this is really not okay. 
		...
		//Yao's printing statements for debugging purposes
		/*std::cout << "\n";
		std::cout << "lig_ali_carb_info size: " << m.lig_ali_carb_info.size() << std::endl;
		std::cout << "rec_ali_carb_info size: " << m.rec_ali_carb_info.size() << std::endl;
		std::cout << "lig_ar_ring_info: " << m.lig_ar_ring_info.size() << std::endl;
		std::cout << "rec_ar_ring_info: " << m.rec_ar_ring_info.size() << std::endl;
		std::cout << "receptor_residues size: " << m.receptor_residues.size() << std::endl;
		std::cout << "ligand_residues size: " << m.ligand_residues.size() << std::endl;
		std::cout << "ligand_atoms size: " << m.ligand_atoms.size() << std::endl;
		std::cout << "grid_atom_ptrs size: " << m.grid_atom_ptrs.size() << std::endl;
		std::cout << "weight_chpi: " << m.weight_chpi << std::endl;
		std::cout << "chpi_explicit_hydrogen: " << m.chpi_explicit_hydrogen << std::endl;
		std::cout << "\n";*/
	}
};
