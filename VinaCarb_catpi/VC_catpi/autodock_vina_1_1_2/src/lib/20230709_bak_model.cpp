...
void model::build_ar_ring_info() {
	...
	VINA_FOR_IN(i, this->rec_ar_ring_info){
		...
		VINA_FOR_IN(j, attr.atom_indices){
			...
			attr.atom_coord_ptrs.push_back(&(this->grid_atoms[ring_atom_index].coords));
		}
...
fl model::eval_chpi_c_ring(aliphatic_carbon_attribute& c_attr, ring_attribute& r_attr){
	vec* cc = c_attr.c_coord;
	bool this_ring_interacting = false;

	fl dH = 0.00;
	VINA_FOR_IN(i, r_attr.atom_coord_ptrs){
		vec* rc = r_attr.atom_coord_ptrs[i];
		fl r = std::sqrt(sqr(cc->data[0]-rc->data[0]) + sqr(cc->data[1]-rc->data[1]) + sqr(cc->data[2]-rc->data[2]));
	...
	if (this_ring_interacting){
        	//Now, compute horizontal offset
                vec c_centroid = r_attr.centroid - (*cc);
                fl cc_length = magnitude(c_centroid);
                if (cc_length < chpi_c_centroid_dcut){

                	fl dp = c_centroid * r_attr.normal;
                	fl vo = std::abs(dp);
               		fl ho = std::sqrt(sqr(cc_length) - sqr(vo));
                	VINA_CHECK(!std::isnan(ho));

                	if (ho <= chpi_ho_max){
                		TdS = eval_chpi_entropy(ho);
                	}
		}
        }
...
fl model::eval_chpi_h_ring(aliphatic_carbon_attribute& c_attr, ring_attribute& r_attr){
	...
	VINA_FOR_IN(i, c_attr.h_coords){
		...
		vec ch_bond; c_attr.ComputeCHBondVectors(i, ch_bond);
        	...
        	VINA_FOR_IN(j, r_attr.atom_coord_ptrs){
                	vec* rc = r_attr.atom_coord_ptrs[j];
...
fl model::eval_chpi_c(){
	...
	VINA_FOR_IN(i, this->lig_ali_carb_info){
		aliphatic_carbon_attribute& c_attr = this->lig_ali_carb_info[i];
		VINA_FOR_IN(j, this->rec_ar_ring_info){
			ring_attribute& r_attr = this->rec_ar_ring_info[j];
			lig_rec_chpi += eval_chpi_c_ring(c_attr, r_attr);
		}
	}

	fl rec_lig_chpi = 0.00;
	VINA_FOR_IN(i, this->rec_ali_carb_info){
		aliphatic_carbon_attribute& c_attr = this->rec_ali_carb_info[i];
		VINA_FOR_IN(j, this->lig_ar_ring_info){
			ring_attribute& r_attr = this->lig_ar_ring_info[j];
                        rec_lig_chpi += eval_chpi_c_ring(c_attr, r_attr);
		}
	}
...
fl model::eval_chpi_h(){
        ...
        VINA_FOR_IN(i, this->lig_ali_carb_info){
                ...
                VINA_FOR_IN(j, this->rec_ar_ring_info){
                        ring_attribute& r_attr = this->rec_ar_ring_info[j];
                        lig_rec_chpi += eval_chpi_h_ring(c_attr, r_attr);
                }
        }
        ...
        VINA_FOR_IN(i, this->rec_ali_carb_info){
                ...
                VINA_FOR_IN(j, this->lig_ar_ring_info){
                        ring_attribute& r_attr = this->lig_ar_ring_info[j];
                        rec_lig_chpi += eval_chpi_h_ring(c_attr, r_attr);
                }
        }
