...
fl naive_non_cache::eval(const model& m, fl v) const { // needs m.coords
	...
	sz n = num_atom_types(p->atom_typing_used());
        ...
        /*int bonded_to_HD_count_1 = 0, bonded_to_HD_count_2 = 0, bonded_to_het_count_1 = 0, bonded_to_het_count_2 = 0;
        int num_effective_pairs = 0;*/
	VINA_FOR(i, m.num_movable_atoms()) {
		fl this_e = 0;
		const atom_vc& a = m.atoms[i];
                //if (m.bonded_to_HD(a)) bonded_to_HD_count_1++;
                //if (m.bonded_to_heteroatom(a)) bonded_to_het_count_1++;
		sz t1 = a.get(p->atom_typing_used());
		if(t1 >= n) continue;
		const vec& a_coords = m.coords[i];

		VINA_FOR_IN(j, m.grid_atoms) {
			const atom_vc& b = m.grid_atoms[j];
                        /*if (m.bonded_to_HD(b)){
                            bonded_to_HD_count_2++;
                        }
                        if (m.bonded_to_heteroatom(b)){
                            bonded_to_het_count_2++;
                        }*/
			...
			fl r2 = sqr(r_ba);
			if(r2 < cutoff_sqr) {
                                //num_effective_pairs++; //Yao's debugging code
	...
	//Might also put chpi and future terms here. Actually, more appropriate. 
	return e;
}
